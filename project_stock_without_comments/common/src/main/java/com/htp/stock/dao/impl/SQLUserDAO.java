...
public class SQLUserDAO implements UserDAO {
    ...
    private static final String LAST_INSERT_ID = "SELECT last_insert_id() as lastId FROM user";
    ...
    private static final String UPDATE_USER = "UPDATE user SET name= ?, surname= ?, login= ?, password= ?, " +
            "phone_number= ?, email= ?, creation_date= ?, type = ? where user_id = ? ";
...
//            "email, creation_date, type
    ...
    public Long create(User entity) throws DaoException {
        try (Connection connect = pool.getConnection();
            ...
            statement.setString(7, entity.getCreationDate());
            statement.setString(8, entity.getType());

            statement.executeUpdate();

            ResultSet set = statementThird.executeQuery();
            set.next();
            return set.getLong(LAST_ID);
...
//            return set.getInt(LAST_ID);
    ...
    private User getEntry(ResultSet set) throws DaoException {
        try {
            ...
            user.setCreationDate(set.getString(CREATION_DATE));
            return user;
