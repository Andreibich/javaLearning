...
public class SQLReceiptDAO implements ReceiptDAO {
    ...
    private static final String SELECT_ALL = "select receipt.receipt_id, companies.company_name, product_catalog.product_name, " +
    ...
    public List<Receipt> findAll() throws DaoException {
        ...
        try (Connection connect = pool.getConnection();
            ...
            while (set.next()) {
                receiptList.add(getEntrySelectAll(set));
            }
    ...
    public Long create(Receipt entity) throws DaoException {
        try (Connection connect = pool.getConnection();
            ...
            statement.setString(1, entity.getReceiptDate());
    ...
    private Receipt getEntry(ResultSet set) throws DaoException {
        try {
            ...
            receipt.setReceiptDate(set.getString(RECEIPT_DATE));
    ...
    private Receipt getEntrySelectAll(ResultSet set) throws DaoException {
        try {
            Receipt receipt = new Receipt();
            receipt.setReceiptId(set.getLong(RECEIPT_ID));
